...
public final class BaritoneMiningBackend implements MiningBackend {
    ...
    public void start(List<String> blockIds, int quantity) {
        ...
        try {
            ...
            applyHumanizedLookSettings(loader);
    ...
    private static void applyHumanizedLookSettings(ClassLoader loader) {
        try {
            Class<?> apiType = Class.forName(API_CLASS, true, loader);
            Object settings = invoke(apiType.getMethod("getSettings"), null);
            if (settings != null) {
                // Camera & rotation realism
                setSetting(settings, "smoothLook", true);
                setSetting(settings, "smoothLookTicks", 15);
                setSetting(settings, "elytraSmoothLook", true);
                setSetting(settings, "randomLooking", 0.02D);
                setSetting(settings, "remainWithExistingLookDirection", true);

                // Anti-cheat compliance & movement realism
                ...
                setSetting(settings, "sprintInWater", false);
                setSetting(settings, "strictLiquidCheck", true);

                // Humanized mining, tool swaps & interaction delays
                setSetting(settings, "autoTool", true);
                setSetting(settings, "preferSilkTouch", true);
                setSetting(settings, "ticksBetweenInventoryMoves", 4);
                setSetting(settings, "inventoryMoveOnlyIfStationary", true);
                setSetting(settings, "blockBreakSpeed", 4);
                setSetting(settings, "rightClickSpeed", 4);

                // Safety, fall damage & mob avoidance
                setSetting(settings, "maxFallHeightNoWater", 3);
                setSetting(settings, "avoidance", true);
                setSetting(settings, "mobAvoidanceRadius", 8);
                setSetting(settings, "avoidUpdatingFallingBlocks", true);
                setSetting(settings, "pauseMiningForFallingBlocks", true);
                setSetting(settings, "itemSaver", true);
                setSetting(settings, "itemSaverThreshold", 20);
            }
        } catch (Exception ignored) {
            // Best effort; older or modified Baritone builds remain usable.
        }
    }

    private static void setSetting(Object settings, String fieldName, Object value) {
        try {
